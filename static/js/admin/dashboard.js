  ...
  try {
    ...
    charts.so = new Chart(document.getElementById('chartSOStatus'), {
      ...
      data: {
        ...
        datasets: [{data: soData, backgroundColor:['#f59e0b','#10b981','#ef4444'], borderWidth:0}]
      },
      options: { responsive:true, maintainAspectRatio:false,
    ...
    charts.ship = new Chart(document.getElementById('chartShipment'), {
      ...
      data: {
        ...
        datasets: [{data: shipData, backgroundColor:['#94a3b8','#0ea5e9','#10b981'], borderWidth:0}]
      },
      options: { responsive:true, maintainAspectRatio:false,
        plugins:{ legend:{position:'bottom'} }, cutout:'65%' }
    });
    ...
    try {
      ...
      if(badge && nr.pending_count>0){badge.style.display='inline';badge.textContent=nr.pending_count;}
    } catch(e) {}
...
function destroyChart(id){
  ...
  if (charts[id.replace('chart','').toLowerCase()]) {
    try { charts[id.replace('chart','').toLowerCase()].destroy(); } catch(e){}
  }
}
