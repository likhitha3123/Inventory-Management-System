...
// ── Init ───────────────────────────────────────────────
async function init() {
  document.querySelector('#soTable thead tr').innerHTML = COLS.map(c => `<th>${c}</th>`).join('');

  const [cr, sr, str] = await Promise.all([
    fetch('/admin/api/customers?dropdown=1').then(r => r.json()),
    fetch('/admin/api/sites?dropdown=1').then(r => r.json()),
  ...
  allCustomers.forEach(c => { custEl.innerHTML += `<option value="${c.customer_id}">${c.customer_id}</option>`; });
  var stateEl = document.getElementById('soState');
  allStates.forEach(s => { stateEl.innerHTML += `<option value="${s.id}">${s.name}</option>`; });
...
function filterSites() {
  var stateId = parseInt(document.getElementById('soState').value) || 0;
  var filtered = stateId ? allSites.filter(s => s.state_id === stateId) : allSites;
  var siteEl = document.getElementById('soSite');
  siteEl.innerHTML = '<option value="">— Select Site —</option>' + filtered.map(s => `<option value="${s.site_id}">${s.site_id} – ${s.site_name}</option>`).join('');
...
async function onSiteChange() {
  ...
  var d = await fetch('/admin/api/inventory-by-site?site_id=' + siteId).then(r => r.json());
...
async function submitOrder() {
  ...
  try {
    var res = await fetch('/admin/api/sales-orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customer_id: custId, site_id: siteId, items: items }) });
...
function resetForm() {
  ...
  document.getElementById('soState').value = '';
  filterSites();
  document.getElementById('productsContainer').innerHTML = '';
  document.getElementById('orderTotal').textContent = '0.00';
  document.getElementById('siteMsg').textContent = '';
  ...
  document.getElementById('tbody').innerHTML = items.length
    ? items.map(o => {
        ...
        if (o.status === 'pending') {
          ...
          actCell =  `<div class="action-btns"><span class="badge badge-success" style="font-size:11px;">✓ Complete</span></div>`;
...
async function viewOrder(id) {
  ...
  if (o.status === 'pending') {
    ...
    actDiv.innerHTML = `
      <button class="btn btn-outline" onclick="closeModal('viewModal')">Close</button>
      ${o.status === 'delivered' ? `<button class="btn" style="background:#fff7ed;border:1.5px solid #fed7aa;color:#ea580c;padding:8px 14px;border-radius:8px;font-size:12px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;gap:6px;" onclick="closeModal('viewModal');openReturnModal('sales','/admin')">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10"/></svg> Return
      </button>` : ''}
      <button class="btn-invoice" onclick="downloadSOInvoice(${o.id})">
        <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
        Download Invoice
